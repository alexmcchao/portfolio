...
df2 = pd.read_sql("SELECT [SalesKitNo], [ComponentItemCode], [QuantityPerAssembly] FROM xxx", db)

df3 = pd.read_sql("SELECT [ItemCode],[ShipWeight],[UDF_HEIGHT],[UDF_LENGTH],[UDF_WIDTH] FROM xxx", db)
...
newdf2["all"] = newdf2["ShipWeight"] + "*" + newdf2["UDF_HEIGHT"] + "*" + newdf2["UDF_LENGTH"]+ "*" + newdf2["UDF_WIDTH"].map(str)
...
for i in range(1, newdf2['count'].max()+1):
    ...
    x.rename(columns={x.columns[0]: "Box"+str(i)+" Ship Weight",
                                            x.columns[1]: "Box"+str(i)+" HEIGHT",
                                           x.columns[2]: "Box"+str(i)+" LENGTH",
                                           x.columns[3]: "Box"+str(i)+" WIDTH"}, inplace = True)
...
data_format4 = workbook.add_format({'bg_color': '#E4FFB8'})
indicator = 1
worksheet.set_column(0, 0, cell_format = data_format4)
